...
from manip.model.transformer_hand_manip_cond_diffusion_model_gomo import CondGaussianDiffusion
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.num_objs_list = [2,3] # change number of objects for training here
    ...
    def prep_dataloader(self):
        ...
        train_datasets = []
        val_datasets = []
        for no in self.num_objs_list:
            train_datasets.append(OMODataset(data_type="train", num_objs=no))
            val_datasets.append(OMODataset(data_type="validation", num_objs=no))

        self.dss = train_datasets 
        self.val_dss = train_datasets
        self.dls = []
        self.val_dls = []
        for noi in range(len(self.num_objs_list)):
            self.dls.append(cycle(data.DataLoader(self.dss[noi], batch_size=self.batch_size, shuffle=True, pin_memory=True, num_workers=2)))
            self.val_dls.append(cycle(data.DataLoader(self.val_dss[noi], batch_size=self.batch_size, shuffle=False, pin_memory=True, num_workers=2)))
    ...
    def train(self):
        ...
        for idx in range(init_step, self.train_num_steps):
            ...
            noi = idx % len(self.num_objs_list)

            nan_exists = False # If met nan in loss or gradient, need to skip to next data. 
            for i in range(self.gradient_accumulate_every):
                data_dict = next(self.dls[noi])

                # print("train noi:", noi)

                data = data_dict['hands_positions_seq'].cuda()
                batch_size, num_frames, num_hands, xyz = data.shape 
                data = data.reshape(batch_size, num_frames, num_hands*xyz)
                    
                objs_data_conds = []
                for oi in range(self.num_objs_list[noi]):
                    obj_bps_data = data_dict[f"bps_object_geo_seq_o{oi+1}"].cuda()
                    obj_bps_data = obj_bps_data.reshape(obj_bps_data.size(0), obj_bps_data.size(1),-1)
                    obj_center_pos = data_dict[f"obj_center_seq_o{oi+1}"].cuda() # BS X T X 3 
                    obj_data_cond = torch.cat((obj_center_pos, obj_bps_data), dim=-1) # BS X T X (3+1024*3)
                    obj_data_cond = obj_data_cond.to(torch.float32)
                    objs_data_conds.append(obj_data_cond)
                ...
                # FP32 for preciser computation and numerical stability
                    ...
                    loss_diffusion = self.model(data, objs_data_conds, cond_mask=None, padding_mask=padding_mask)
            ...
            if self.step != init_step and self.step % 10 == 0: # evaluate in every 10 steps
                ...
                with torch.no_grad():
                    vnoi = validation_idx % len(self.num_objs_list)
                    validation_idx = validation_idx + 1
                # evaluation
                    val_data_dict = next(self.val_dls[vnoi])

                    # print("validation noi:", vnoi)
                    ...
                    objs_data_conds = []
                    for oi in range(self.num_objs_list[vnoi]):
                        obj_bps_data = val_data_dict[f"bps_object_geo_seq_o{oi+1}"].cuda()
                        obj_bps_data = obj_bps_data.reshape(obj_bps_data.size(0), obj_bps_data.size(1),-1)
                        obj_center_pos = val_data_dict[f"obj_center_seq_o{oi+1}"].cuda() # BS X T X 3 
                        obj_data_cond = torch.cat((obj_center_pos, obj_bps_data), dim=-1) # BS X T X (3+1024*3)
                        obj_data_cond = obj_data_cond.to(torch.float32)
                        objs_data_conds.append(obj_data_cond)
                    ...
                    # Get validation loss 
                    val_loss_diffusion = self.model(val_data, objs_data_conds, cond_mask=None, padding_mask=padding_mask)
...
def parse_arg():
    ...
    parser.add_argument('--use_wandb', action="store_true", default=False, help='add to use wandb')
